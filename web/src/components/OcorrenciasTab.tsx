...
export function OcorrenciasTab({ planoId }: { planoId: number }) {
  ...
  useEffect(() => {
    ...
    api
      .get<{ tratamento_dia_nao_util: string; ocorrencias: Ocorrencia[] }>(
        `/planos/${planoId}/ocorrencias?inicio=${inicio}&fim=${fim}`
      )
      .then(setDados)
      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-header__desc">
            Calculadas pelo motor de recorrência a partir da data-base do plano — a geração de OS em lote (fase
            seguinte) usará este mesmo cálculo.
          </div>
        </div>
              ...
              {dados.ocorrencias.length === 0 && (
                <tr>
                  <td colSpan={3} className="empty-state">
                    Nenhuma ocorrência prevista neste período (plano inativo, fora de vigência, ou horizonte muito
                    curto para a periodicidade).
                  </td>
                </tr>
              )}
            </tbody>
