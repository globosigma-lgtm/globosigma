...
import type { ResultadoSimulacaoInspecao } from "../types";
...
export function GerarLoteInspecaoImprimir() {
  ...
  const classePeriodicidade = params.get("classePeriodicidade");
  ...
  useEffect(() => {
    ...
    api
      .post<ResultadoSimulacaoInspecao>("/inspecoes/lotes/simular", { ano, semana_inicio: semanaInicio, semana_fim: semanaFim, filtros })
      .then(setResultado)
      .catch((e) => setErro(e instanceof ApiError ? e.message : "Não foi possível gerar o relatório."));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ano, semanaInicio, semanaFim, ativoId, classePeriodicidade, texto]);
    ...
    <div className="lote-print">
      ...
      <div className="lote-print__header">
        <div className="titulo">
          ...
          <div>
            <h1>Relatório de geração de inspeções em lote</h1>
            <div className="sub">
      ...
      <div className="lote-print__resumo">
        <div className="item">
          <label>Novas inspeções a gerar</label>
        ...
        <tbody>
          {resultado.ocorrencias.map((o) => (
            <tr key={o.chave_idempotencia}>
              <td>{o.ativo_nome}</td>
              <td>{o.tag}</td>
              <td>{o.classe_periodicidade ? ROTULO_CLASSE_PERIODICIDADE[o.classe_periodicidade] : "—"}</td>
