...
export function GerarLoteImprimir() {
  ...
  useEffect(() => {
    ...
    api
      .post<ResultadoSimulacao>("/geracao-lote/simular", { data_inicio: dataInicio, data_fim: dataFim, filtros })
      .then(setResultado)
      .catch((e) => setErro(e instanceof ApiError ? e.message : "Não foi possível gerar o relatório."));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dataInicio, dataFim, ativoId, tipoManutencao, texto]);
    ...
    <div className="lote-print">
      ...
      <div className="lote-print__header">
        <div className="titulo">
          ...
          <div>
            <h1>Relatório de geração de OS em lote</h1>
            <div className="sub">
              Período: {formatarData(resultado.data_inicio)} a {formatarData(resultado.data_fim)}
            </div>
          </div>
        </div>
        <div className="meta">
      ...
      <table className="lote-print__tabela">
        ...
        <tbody>
          {resultado.ocorrencias.map((o) => {
            ...
            return (
              <tr key={o.chave_idempotencia}>
                <td>{o.ativo_nome}</td>
                <td>{consolidada ? `${o.planos.length} atividades: ${o.planos.map((p) => p.codigo).join(", ")}` : `${o.plano_codigo} — ${o.plano_nome}`}</td>
          ...
          {resultado.ocorrencias.length === 0 && (
            <tr>
              <td colSpan={9} className="lote-print__linha-vazia">
