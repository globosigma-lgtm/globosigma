...
import type { AtivoComArvore, LoteGeracaoInspecao, ResultadoSimulacaoInspecao, StatusLoteInspecao } from "../types";
import { ROTULO_CLASSE_PERIODICIDADE } from "../components/PlanoInspecaoFormModal";
import { semanaDoAno } from "../lib/semanas";

const ROTULO_STATUS_LOTE: Record<StatusLoteInspecao, string> = {
  simulado: "Simulado",
  confirmado: "Confirmado",
  revertido: "Revertido",
};

const BADGE_STATUS_LOTE: Record<StatusLoteInspecao, string> = {
...
export function GerarLoteInspecao() {
  const { pode } = useAuth();
  ...
  const [resultado, setResultado] = useState<ResultadoSimulacaoInspecao | null>(null);
  ...
  const [lotes, setLotes] = useState<LoteGeracaoInspecao[] | null>(null);
  const [revertendoId, setRevertendoId] = useState<number | null>(null);
  const [erroHistorico, setErroHistorico] = useState<string | null>(null);
  const [loteParaReverter, setLoteParaReverter] = useState<LoteGeracaoInspecao | null>(null);
  ...
  function carregarHistorico() {
    api
      .get<{ lotes: LoteGeracaoInspecao[] }>("/inspecoes/lotes")
  ...
  function filtrosAtuais() {
    const filtros: { ativoId?: number; classePeriodicidade?: string; texto?: string } = {};
    if (ativoId) filtros.ativoId = Number(ativoId);
    if (classePeriodicidade) filtros.classePeriodicidade = classePeriodicidade;
  ...
  async function aoSimular() {
    ...
    setSimulando(true);
    try {
      const r = await api.post<ResultadoSimulacaoInspecao>("/inspecoes/lotes/simular", {
        ano,
      ...
      setResultado(r);
      const sugestao = await api.get<{ codigo: string }>("/inspecoes/lotes/codigo-sugerido");
  ...
  async function aoConfirmar() {
    ...
    setConfirmando(true);
    try {
      const criado = await api.post<{ lote: LoteGeracaoInspecao }>("/inspecoes/lotes", {
        codigo: codigo.trim(),
        ano: resultado.ano,
      ...
      const confirmado = await api.post<{ lote: LoteGeracaoInspecao; ordens_criadas: number; ocorrencias_ignoradas: number }>(
        `/inspecoes/lotes/${criado.lote.id}/confirmar`
      );
      setSucesso(
        `Lote ${confirmado.lote.codigo} confirmado: ${confirmado.ordens_criadas} inspeção(ões) gerada(s)` +
  ...
  async function confirmarReversao() {
    ...
    try {
      const r = await api.post<{ revertidas: number; naoRevertidas: number }>(`/inspecoes/lotes/${lote.id}/reverter`);
      setSucesso(
  ...
  const podeGerar = pode("inspecoes", "criar");
  const podeAprovar = pode("inspecoes", "aprovar");
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Gerar inspeções em lote</h1>
          <div className="page-header__desc">
            Calcula as ocorrências dos planos de inspeção ativos e classificados num intervalo de semanas do ano e
            gera as OS de uma vez
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: "16px" }}>
        ...
        <div className="filters-bar">
          ...
          <div className="field">
            ...
            <input id="f-texto" className="input" placeholder="Código, TAG ou equipamento" value={texto} onChange={(e) => setTexto(e.target.value)} />
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: "16px" }}>
          ...
          {resultado.sobrecargas.length > 0 && (
            <div className="alert alert--warning" style={{ marginBottom: "12px" }}>
              <strong>Possível sobrecarga de inspetores:</strong>
              <ul>
          ...
          {resultado.total_novas > 0 && podeAprovar && (
            <div className="filters-bar" style={{ alignItems: "flex-end" }}>
              ...
              <button type="button" className="btn btn--primary" onClick={aoConfirmar} disabled={confirmando || !codigo.trim()}>
                {confirmando ? "Confirmando…" : `Confirmar geração de ${resultado.total_novas} inspeção(ões)`}
              </button>
            </div>
          )}
          {resultado.total_novas > 0 && !podeAprovar && (
        ...
        <div className="modal-overlay" onMouseDown={(e) => e.target === e.currentTarget && setLoteParaReverter(null)}>
          <div className="modal" style={{ maxWidth: "440px" }}>
            ...
            <p style={{ color: "var(--c-n-700)", marginBottom: "8px" }}>
              Tem certeza que deseja reverter <strong>{loteParaReverter.codigo}</strong>? As OS geradas por este lote
              que ainda estiverem com status "programada" serão excluídas; as que já saíram desse status são
              mantidas.
            </p>
            {erroHistorico && (
