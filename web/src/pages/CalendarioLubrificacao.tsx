...
import type { AtivoComArvore, LinhaCalendarioLubrificacao } from "../types";
...
export function CalendarioLubrificacao() {
  const [ano, setAno] = useState(new Date().getFullYear());
  const [linhas, setLinhas] = useState<LinhaCalendarioLubrificacao[] | null>(null);
  ...
  function carregar() {
    ...
    api
      .get<{ linhas: LinhaCalendarioLubrificacao[] }>(`/lubrificacao/calendario?${params.toString()}`)
      .then((r) => setLinhas(r.linhas))
      .catch((e) => setErro(e instanceof ApiError ? e.message : "Não foi possível carregar o calendário de lubrificação."));
  }

  useEffect(carregar, [ano, ativoId, periodicidade]);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Calendário de lubrificação</h1>
          <div className="page-header__desc">
            Ocorrências previstas dos pontos de lubrificação ativos, por número de semana do ano — calendário separado
            do de planos de manutenção
          </div>
        </div>
            ...
            {linhas?.map((l) => {
              ...
              return (
                <tr key={l.ponto_id}>
                  ...
                  <td>
                    <span className="mono">{l.ponto_codigo}</span> — {l.ponto_descricao}
                  </td>
            ...
            {linhas?.length === 0 && (
              <tr>
                <td colSpan={totalSemanas + 3} className="empty-state">
                  Nenhum ponto de lubrificação ativo com ocorrências previstas em {ano} para os filtros atuais.
                </td>
              </tr>
            )}
          </tbody>
